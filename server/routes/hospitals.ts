...
router.post('/', requireAuth, validate(CreateHospital), asyncHandler(async (req, res) => {
	...
	const insertRes: any = await (db as any).insert(hospitals).values(payload).execute();
	const id = insertRes.insertId as number;
	const record = await (db as any).select().from(hospitals).where(eq((hospitals as any).id, id)).limit(1);
	res.status(201).json({ success: true, data: record[0], message: 'Hospital created' });
}));
...
router.get('/', validate(ListQuery, 'query'), asyncHandler(async (req, res) => {
	...
	const data = await (db as any).select().from(hospitals).limit(limit).offset(offset);
	const [{ total }] = await (db as any).select({ total: (db as any).raw('COUNT(*)') }).from(hospitals) as any[];
...
router.put('/:id', requireAuth, validate(UpdateHospital), asyncHandler(async (req, res) => {
	const id = Number(req.params.id);
	await (db as any).update(hospitals).set(req.body as any).where(eq((hospitals as any).id, id));
	res.json({ success: true, message: 'Updated' });
}));

router.delete('/:id', requireAuth, asyncHandler(async (req, res) => {
	const id = Number(req.params.id);
	await (db as any).delete(hospitals).where(eq((hospitals as any).id, id));
