...
router.post('/', requireAuth, validate(CreateQuiz), asyncHandler(async (req, res) => {
	...
	const insertResult: any = await db.insert(quizzes).values(payload).execute();
	const insertedId = insertResult.insertId as number;
	const [record] = await db.select().from(quizzes).where(eq(quizzes.id, insertedId)).limit(1);
...
router.get('/:id', asyncHandler(async (req, res) => {
	...
	const record = await db.select().from(quizzes).where(eq(quizzes.id, id)).limit(1);
...
router.put('/:id', requireAuth, validate(UpdateQuiz), asyncHandler(async (req, res) => {
	const id = Number(req.params.id);
	await db.update(quizzes).set(req.body as any).where(eq(quizzes.id, id)).execute();
	res.json({ success: true, message: 'Updated' });
}));

router.delete('/:id', requireAuth, asyncHandler(async (req, res) => {
	const id = Number(req.params.id);
	await db.delete(quizzes).where(eq(quizzes.id, id)).execute();
